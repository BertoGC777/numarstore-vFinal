...
export const adminMiddleware = async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    await getDatabase();
    const user = dbGet<{ role: string }>("SELECT role FROM users WHERE id = ?", [decoded.id]);
...
export const refreshMiddleware = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await getDatabase();
    const user = dbGet<{ id: string; name: string; email: string }>("SELECT id, name, email FROM users WHERE id = ?", [decoded.id]);
