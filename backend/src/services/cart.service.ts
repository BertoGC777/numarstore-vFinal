...
export async function addToCart(userId: string | null, productId: string, color: string, size: string, quantity = 1) {
  await getDatabase();
  const existing = await dbGet("SELECT quantity FROM cart_items WHERE user_id = ? AND product_id = ? AND color = ? AND size = ?",
    [userId, productId, color, size]);
  if (existing) {
    await dbRun("UPDATE cart_items SET quantity = quantity + ? WHERE user_id = ? AND product_id = ? AND color = ? AND size = ?",
      [quantity, userId, productId, color, size]);
    return;
  }
  await dbRun("INSERT INTO cart_items (user_id, product_id, color, size, quantity) VALUES (?, ?, ?, ?, ?)",
    [userId, productId, color, size, quantity]);
}

export async function updateCartItem(id: number, quantity: number) {
  await getDatabase();
  if (quantity <= 0) { await dbRun("DELETE FROM cart_items WHERE id = ?", [id]); return; }
...
export async function getCartTotal(userId: string | null): Promise<number> {
  await getDatabase();
  const r = await dbGet<{ total: number | null }>("SELECT SUM(p.price_pix * ci.quantity) as total FROM cart_items ci JOIN products p ON ci.product_id = p.id WHERE ci.user_id = ?", [userId]);
  return r?.total || 0;
}