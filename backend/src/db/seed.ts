import { dbRun, dbGet, getDatabase } from "./sqlite";
...
export async function seedAll() {
   await getDatabase();
   const count = dbGet<{ cnt: number }>("SELECT COUNT(*) as cnt FROM products");
  if (count && count.cnt > 0) return;

  for (const p of productData) {
    const id = crypto.randomUUID();
dbRun(
       "INSERT INTO products (id, slug, name, description, category, subcategory, price_pix, price_card, old_price, is_new, is_sale, discount, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 0, 0, ?)",
    ...
    for (const c of p.colors) {
      ...
      dbRun("INSERT INTO product_images (product_id, url, color, color_hex, sort_order) VALUES (?, ?, ?, ?, ?)",
        [id, `/images/${p.slug}-0.jpg`, c, hex, 0]);
      dbRun("INSERT INTO product_colors (product_id, name, hex) VALUES (?, ?, ?)", [id, c, hex]);
    }
    for (const s of p.sizes) {
      dbRun("INSERT INTO product_sizes (product_id, size) VALUES (?, ?)", [id, s]);
    }
  }
